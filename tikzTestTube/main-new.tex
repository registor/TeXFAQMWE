...
\newcommand{\beaker}[3][]{
  \begin{scope}[shift={(#2)},glassware,#1]
    \ifnonzero{#3}{
      \clip[draw, clip] (-1.4, 3) to[rounded corners=2pt]++(0.1,-0.1)
...
\newcommand{\flask}[3][]{
  \begin{scope}[shift={(#2)},glassware,#1]
    ...
    \ifnonzero{#3}{
      ...
      \pgfmathsetmacro{\i}{\m - \n}
      % 绘制液面矩形并被烧瓶裁剪
      \filldraw (-1.0, -3.5pt) rectangle (1.0, \i);
      % 绘制液面椭圆标志
      % 定义水平线
      \path[name path=P2](-1.0, \i)--++(2,0);
      % 求交点
...
\newcommand{\testtube}[3][]{
  \begin{scope}[shift={(#2)},glassware,#1]
    \ifnonzero{#3}{
      \clip(-0.4, 3) to[rounded corners=2pt]++(0.1,-0.1)
...
\begin{figure}[h!]
  \begin{center}
    \begin{tikzpicture}
      \begin{scope}
        \testtube{0.0,0.0}{0.00}
      \end{scope}
      \begin{scope}[shift={(2.5,0)}]
        \testtube[rotate=15, draw=red, fill=violet!50]{0,0}{0.25}
        \metalcoil[rotate=15, fill=teal!25, scale=0.8]{0,0.1}
      \end{scope}      
